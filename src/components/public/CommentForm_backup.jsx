...
const CommentForm = ({ postId, onSubmit, submitting }) => {
	...
	useEffect(() => {
		...
		console.log("Verificando cooldown:", { lastComment });
		
		if (lastComment) {
			const timePassed = Date.now() - parseInt(lastComment);
			const cooldown = 5 * 60 * 1000; // 5 minutos em milissegundos

			console.log("Time passed:", timePassed, "Cooldown:", cooldown);

			if (timePassed < cooldown) {
				const minutesRemaining = Math.ceil(
					(cooldown - timePassed) / 60000,
				);
				console.log("Cooldown ativo, minutos restantes:", minutesRemaining);
				
				setCooldownTime(minutesRemaining);
				setShowCooldownMessage(true); // Mostra mensagem apenas se já comentou antes

				// Mostra toast imediatamente ao carregar a página se ainda estiver em cooldown
				toast.error(
				...
				console.log("Cooldown expirado, limpando localStorage");
				localStorage.removeItem("lastCommentTime");
			}
		} else {
			console.log("Nenhum comentário anterior encontrado");
		}
	}, []);
	...
	const handleSubmit = async (e) => {
		...
		if (showCooldownMessage && cooldownTime > 0) {
			toast.error(
				`Aguarde ${cooldownTime} minutos para comentar novamente.`,
			);
			return;
		}
	...
	return (
		<div className="bg-gray-50 rounded-lg p-6 mb-8">
			...
			{showCooldownMessage && cooldownTime > 0 ? (
				<>
					{console.log("Renderizando cooldown UI:", { showCooldownMessage, cooldownTime })}
					<div className="bg-orange-50 text-orange-800 p-4 rounded-lg flex items-start space-x-3 mb-4">
						...
						<div>
							...
							<p className="text-sm mt-1">
								Para evitar spam, permitimos um comentário a cada 5 minutos. Por favor, aguarde cerca de{" "}
								<strong>{cooldownTime} minutos</strong> para enviar novamente.
							</p>
						</div>
					</div>
				...
				<>
					{console.log("Renderizando formulário normal:", { showCooldownMessage, cooldownTime })}
