...
const MAX_COMMENTS_PER_HOUR = import.meta.env.DEV ? 100 : 3;
...
export const validateHoneypot = (formData) => {
	// Skip validation in development to avoid testing issues
...
export const secureValidateComment = (formData) => {
	...
	if (!rateLimit.allowed) {
		throw new Error(
			`Limite de comentários excedido. Aguarde ${rateLimit.resetIn} minutos antes de comentar novamente.`,
		);
	}
