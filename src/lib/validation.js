...
export const validateName = (name) => {
	// Mínimo 2 caracteres, máximo 100, apenas letras e espaços
...
export const validateComment = (comment) => {
	...
	return comment && comment.trim().length >= 10 && comment.trim().length <= 2000;
};
...
export const getFieldError = (fieldName, value) => {
	switch (fieldName) {
		...
		case 'password':
			...
			if (!validatePassword(value)) return 'A senha deve ter pelo menos 6 caracteres';
			return null;
		
		case 'name':
			if (!value) return 'O nome é obrigatório';
			if (!validateName(value)) return 'Digite um nome válido (mínimo 2 caracteres)';
			return null;
		
		case 'content':
			if (!value) return 'O comentário é obrigatório';
			if (!validateComment(value)) return 'O comentário deve ter entre 10 e 2000 caracteres';
			return null;
